            ...
            for k in filter_matrix:
                if i + k[0] < 0 or j + k[1] < 0 or j + k[1] >= image.shape[1] or i + k[0] >= image.shape[0]:
                    ...
                    for n in range(3):
                        ...
                        pixel_num[n] += individual_weight

            for n in range(3):
                output_image[i, j, n] = (neighbourhood_total[n] // pixel_num[n]) 
    output_image = cv2.cvtColor(output_image, cv2.COLOR_LAB2BGR)
    image = cv2.cvtColor(image, cv2.COLOR_LAB2BGR)


    return output_image
    #cv2.imwrite("intensity="+str(sigma_intensity)+", space="+str(sigma_space)+", filter="+str(filter_size) +
    #            ".jpg", output_image)
    #cv2.imwrite("difference_intensity="+str(sigma_intensity)+", space="+str(sigma_space)+", filter="+str(filter_size) +
    #            ".jpg", image - output_image)
...
def main():
    if len(sys.argv) > 1:
        ...
        img = cv2.imread("source/test3a.jpg")
        img = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
        img2 = cv2.imread("source/test3b.jpg")
